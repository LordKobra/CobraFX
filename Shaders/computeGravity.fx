...
void gravityMain(uint3 id : SV_DispatchThreadID, uint3 tid : SV_GroupThreadID)
{
	...
	for(int y = 0; y < GRAVITY_HEIGHT; y++)
	{
		...
		float4 rgbval =  tex2Dfetch(ReShade::BackBuffer, int4(id.x*GRAVITY_RES_X, yi*GRAVITY_RES_Y, 0, 0)); //access
	...
	{
		if (y != finalist[y]) {
			float4 storeVal = tex2Dfetch(ReShade::BackBuffer, int4(id.x, finalist[y] * GRAVITY_RES_Y, 0, 0)); // access
...
void downsample_gravity(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 fragment : SV_Target)
{
	...
	float depthGravity = tex2D(SamplerGravityDepth, texcoord);
