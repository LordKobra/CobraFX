	...
	void sort_color(uint3 id : SV_DispatchThreadID, uint3 tid : SV_GroupThreadID)
	{
		...
		if(tid.y == 0)
		{
			...
			for (i = 0; i < COLOR_HEIGHT; i++)
			{
				...
				float d1 = distance(colortable[i + tid.x*COLOR_HEIGHT].rgb, Color1);
				float d2 = distance(colortable[i + tid.x*COLOR_HEIGHT].rgb, Color2);
				...
				if (!(is_focus && was_focus))
					maskval++;
				was_focus = is_focus;
				colortable[i + tid.x*COLOR_HEIGHT].a = (float)maskval;
